...
export const requestDirectoryPermission = async (): Promise<MediaLibrary.Album | null> => {
    const permission = await MediaLibrary.requestPermissionsAsync()
    if (permission.granted) {
        const album = await MediaLibrary.getAlbumAsync('bobochat')
        if (album) {
            return album
        } else {
            return await MediaLibrary.createAlbumAsync('bobochat')
        }
    }
    return null;
    // if(album){
    ...
    //         const downloadDir = MediaLibrary.getAlbumAsync
